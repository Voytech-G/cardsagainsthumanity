...
const joinRoom = (io, socket, rooms, sessions, roomId, password, callback) => {
    ...
    isLoggedIn(sessions, socket, rooms, socket.sessionId, response => {
        ...
        if(response.status){
            ...
            if(sessions[socket.sessionId].getIsInGame() && sessions[socket.sessionId].getRoomId()){
            ...
            let current_occupation      = Object.keys(rooms[roomId].connectedPlayers).length;
            
            let max_allowed_occupation  = rooms[roomId].maxPlayers;

            logInfo(`number of players in room: ${current_occupation}, max allowed players: ${rooms[roomId].maxPlayers}`);
            ...
            if(!rooms[roomId].isGameReady){
                ...
                if(Object.keys(rooms[roomId].connectedPlayers).length >= 3){
                    ...
                    sendRoomMessage(io, socket, `system`, `System`, `The game can now be started`, roomId,  true);
            ...
            if(!sendRoomMessage(io, socket, 'system', username, `${username} has joined the room`, roomId))
                logError(`failed to send the message, missing username or type or roomid or message`);
            ...
            getRoomInfo(rooms, roomId, response => {
                ...
                if(response.status){
                    ...
                    logError(`error while sending room data of updated room: ${response.err}`);
