...
const { logError, logInfo, logSuccess, logFatal, logWarn } = require(`./logs.js`);
...
const createRoom = (data, sessions, io, socket, rooms, callback) =>{
    ...
    logInfo(`user: ${owner.name} is creating a room which name will be: ${roomName}`);				
    ...
    joinRoom(io, socket, rooms, sessions, roomId, roomPassword, response =>{
        ...
        if(response.status){
            ...
            logSuccess(`successfully joined the created room, all existing rooms: ${JSON.stringify(rooms)}`);
        ...
        // failed to join a room
            ...
            logError(`failed to join given room after creating it, error: ${response.err}`);
            callback({
                status: false,
