...
const signUp = (data, callback) => {
    ...
    // getting username and email field from database
        ...
        if (err) {
            ...
            logError(`Failed to query the database for given username and email in signup-attempt socket, err: ${err}`);
        ...
        // no errors occured, proceeding with signing up process
            ...
            if (results.length === 0) {
                logSuccess('found no users with that username and email, proceeding...');
                ...
                logWarn(`result of validation of sign up data: ${JSON.stringify(result)}`);
            ...
            // found users with that username or email
                ...
                logWarn("We already have user(s) with that username or email in database");
