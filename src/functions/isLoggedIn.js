const {logSuccess, logWarn, logError, logFatal, logInfo } = require(`./logs.js`);
...
const isLoggedIn = (sessions, socket, rooms, sessionId, callback) => {
    ...
    if(socket.authenticated){
        ...
        if(sessions[sessionId]){
            ...
            logError(`socket is authenticated but session of given session id doesn't exist`);
            callback({
                status: false,
    ...
    // socket connection is not authenticated
        ...
        logInfo(`socket.authenticated is false, searching for a session of given session id`);
        ...
        if(!sessions[sessionId]){
            ...
            logInfo(`found no session of session id: ${sessionId}, user is not logged in`);
        ...
        logSuccess(`assigned the existing session id to socket connection, checking if user is in gameroom`);
        ...
        if(sessions[sessionId].getIsInGame() && sessions[sessionId].getRoomId()){
            ...
            if(!rooms[roomId]){
                ...
                logError(`room of given room id doesn't exist, couldnt update users socket id in the room`);

            } else {

                if(!rooms[roomId].connectedPlayers[userId]){

                    logError(`found room of room id from user's session but haven't found the user in that room`);

                } else {

                    logSuccess(`found this user in room hes supposed to be in, updating his socket id`);
