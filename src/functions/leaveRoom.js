const { logSuccess, logError, logWarn, logInfo, logFatal } = require('./logs.js');
    ...
    isLoggedIn(sessions, socket, rooms, sessionId, response => {
        ...
        if(response.status){
            ...
            if(Object.keys(rooms[roomId].connectedPlayers).length === 1){
                ...
                logWarn(`this user is the last user in this room, disconnecting the user and removing the room`);
                ...
                socket.leave(roomId, err => {
                    if(err){
                        ...
                        logSuccess(`successfully removed the room and left the socket.io room as you are the last player in it`);
            ...
            // user leaving the room is not the last user in it
                ...
                logWarn(`this user is not the last user in this room, not removing the room`);
                ...
                if(rooms[roomId].owner.name === username && rooms[roomId].owner.id === userId){

                    logWarn(`user trying to leave the room is the owner, choosing random player to become the new owner`);
                    ...
                    logSuccess(`updated the owner of this room: ${JSON.stringify(rooms[roomId].owner)}, notifying the user that he became one`);

                    sendPrivateMessage(io, `system`, `System`, `Server chose you to be the owner of the room`, new_owner.socketId);
                    ...
                    for(let i = 0;i<uids.length;i++){
                        if(uids[i] !== new_owner.userId){
                            sendPrivateMessage(io, `system`, `System`, `${new_owner.username} is now the owner of the room`, rooms[roomId].connectedPlayers[uids[i]].socketId);
                        }  
                    }  
                }
                ...
                if(rooms[roomId].isGameReady){
                    ...
                    logFatal(`rooms[roomId] isGameReady jest true, checking if there are less than 3 players now`);
                    ...
                    if(Object.keys(rooms[roomId].connectedPlayers).length < 3){
                        ...
                        if(rooms[roomId].isGameStarted){
                            ...
                            sendRoomMessage(io, socket, `system`, `System`, `Game stopped, not enough players in the room`, roomId, true);

                        }
                        
                        logInfo(`the game can not be started anymore, not enough players in the room`);

                        sendRoomMessage(io, socket, `system`, `System`, `Waiting for more players...`, roomId);
                        ...
                        // number of players is lower than 3
                            ...
                            let ownerSocketId = rooms[roomId].connectedPlayers[ownerUserId].socketId;
                ...
                getRoomInfo(rooms, roomId, response => {
                    ...
                    if(response.status){
                        ...
                        logSuccess(`got the data about updated room, sending it to connected players`);
                    ...
                    socket.leave(roomId, err => {
                        ...
                        if(err){
                            ...
                            logError('An error occured while disconnecting the socket from socket.io room');
                            ...
                            logSuccess(`Successfully disconnected from socket.io room of id: ${roomId}`);

                            // getting username of user leaving the room
                            let username = sessions[sessionId].getUsername();

                            if(!sendRoomMessage(io, socket, `system`, `System`, `${username} left the game.`, roomId))
                                logError(`failed to send a message notifying of user leaving the room`);
