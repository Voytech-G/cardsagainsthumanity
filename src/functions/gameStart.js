...
const gameStart = (io, socket, sessions, sessionId, rooms, callback) => {
    ...
    if(!roomId || !sessions[sessionId].getIsInGame()) {
        ...
        logError(`user is logged in but found no room id in his session, cannot start the game`);
    ...
    if(ownerUserId === sessions[sessionId].getUserId()) {
        ...
        if(!sendRoomMessage(io, socket, `system`, `System`, `Game started`, roomId, true)){
