...
const { logSuccess, logError, logWarn, logInfo, logFatal } = require('./src/functions/logs.js');
...
io.on(`connection`, socket => {
	...
	socket.on(`chat-message`, data => {
		...
		if(!sendRoomMessage(io, socket, `user`, data.username, data.message, roomId))
			logError(`failed to send the message, missing username or message or room id`);
	...
	socket.on(`get-roominfo`, (sessionId, callback) => {
		...
		isLoggedIn(sessions, socket, rooms, sessionId, response => {
			...
			if(response.status){
				...
				logInfo(`user is logged in, getting data of room of id: ${roomId} in get-roominfo event`);
			...
			// user is not logged in
				...
				logError(`error while checking if user trying to get room info is logged in: ${JSON.stringify(response.err)}`);
				callback(response);
			}
		});
		...
		isLoggedIn(sessions, socket, rooms, requestSessionId, response => {
			...
			if(response.status){
				...
				createRoom(data, sessions, io, socket, rooms, response => {
					if(response.status)
						logSuccess(`create room callback successful`);
					else
						logError(`create room callback failure, err: ${response.err}`);
					callback(response);
				});
		...
		isLoggedIn(sessions, socket, rooms, sessionId, response => {
			...
			if(response.status){
				...
				if(sessions[sessionId].getUserId() === ownerUserId){
					...
					logFatal(`user: ${sessions[sessionId].getUsername()} is the owner, sending him the room-ready-state`);
				...
				// user asking for room-ready-state is not the owner of the room
					...
					logFatal(`user: ${sessions[sessionId].getUsername()} is not the owner, cannot send him room-ready-state`);
				}
...
;				gameStart(io, socket, sessions, sessionId, rooms, response => {
	...
	socket.on('join-room', (data, callback) => {
		...
		isLoggedIn(sessions, socket, rooms, sessionId, response => {
			...
			if(response.status){
				...
				logWarn(`user: ${sessions[sessionId].getUsername()} is trying to join room of id: ${roomId}`);
	...
	socket.on('leave-room', (sessionId, callback) => {
		...
		isLoggedIn(sessions, socket, rooms, sessionId, response => {
			...
			if(response.status){
				...
				logWarn(`user ${sessions[sessionId].getUsername()} is trying to leave a game room he's in`);
	...
	socket.on('signup-attempt', (data, callback) => {
		...
		logInfo(`recieved data of the new user: ${username} | ${password} | ${email}`);
		...
		if(username && password && email){
			...
			logError(`username, password or email not recieved in signup-attempt socket function`);
	...
	socket.on('signin-attempt', (data, callback) => {
		...
		if(sessionId){
			...
			isLoggedIn(sessions, socket, rooms, sessionId, response => {
				...
				if(response.status){
					...
					logSuccess(`this session id already points to an active session, username: ${sessions[sessionId].getUsername()}`);
				...
				// session id does not point to an active session
					...
					logWarn(`session id: ${sessionId} does not point to an active session, logging in...`);
		...
		// recieved neither of those, aborting 
			...
			logFatal(`recieved session id is neither a correct session id string or an empty string indicating that the user is not logged in, logging in failed.`)
	...
	socket.on(`logout`, (data, callback) => {
		...
		isLoggedIn(sessions, socket, rooms, sessionId, response => {
			...
			if(response.status){
				...
				logInfo(`${sessions[sessionId].username} is currently logged in, logging out`);
	...
	socket.on('disconnect', () => {
		...
		if(socket.authenticated){
			...
			if(!socket.sessionId){
				logError(`session id not in socket connection object but session is apparently authenticated? XD`);
				return;
			} else
				logInfo(`${sessions[socket.sessionId].getUsername()} wants to disconnect`);
			
		} else
			logInfo(`user of socket id: ${socket.id} wants to disconnect`);
